...
// Custom control that allows a user to pick a number between 1-120
class DurationControl: UIControl {
    ...
    var value: Int {
        get {
            ...
            let spots: CGFloat = 119.0  // 120 - 1
            
            return Int(1 + range/(space/spots))
        }
        set {
            if (newValue <= 1) {
                position = offSet
    ...
    override func draw(_ rect: CGRect) {
        ...
        styles[NSAttributedStringKey.font] = UIFont(name: "DINCondensed-Bold", size: self.bounds.height/4 )
        styles[NSAttributedStringKey.foregroundColor] = UIColor.darkGray
        
        // Text
        let duration = "\(Int(value)) seconds"
        
        // Create and draw
        var attribute = NSAttributedString(string: duration, attributes: styles)
        ...
        // M LABEL
        attribute = NSAttributedString(string: "1", attributes: styles)
        ...
        // S LABEL
        attribute = NSAttributedString(string: "120", attributes: styles)
        setter = CTFramesetterCreateWithAttributedString(attribute as CFAttributedString)
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        // Basically just calculates the x cordinate and moves it to it.
